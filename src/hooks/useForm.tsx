...
/** Wraps the useForm, returning inputs components with `control` prop populated and having mode: 'onTouched' in useForm option. */
export function useForm<F extends FieldValues = FieldValues, C extends object = object>(p?: UseFormProps<F, C>): UseFormReturn<F, C> {
  const useFormReturn = useFormInternal({ mode: 'onTouched', ...p });
